...
export default function RtkSurveyCockpit({ deviceId, device, onNotice }) {
  ...
  const adjustSpeedRatio = useCallback(delta => {
    setSpeedRatio(current => Math.min(MAX_SPEED_RATIO, Math.max(0.05, Number((current + delta).toFixed(2)))))
  }, [])
      ...
      <div className="rtk-cockpit-grid">
        ...
        <div className="rtk-drive-card">
          ...
          <div className="rtk-speed-control">
            <span>速度倍率</span><strong>{Math.round(speedRatio * 100)}% · {(controlConfig.maxLinear * speedRatio).toFixed(2)} m/s</strong>
            <div className="rtk-speed-adjuster">
              ...
              />
              <button type="button" onClick={() => adjustSpeedRatio(0.05)} disabled={Boolean(activeDirection) || speedRatio >= MAX_SPEED_RATIO}>+5%</button>
            </div>
          </div>
          <RobotDirectionPad
